...
for i = 1:length(headers)
    ...
    if site_ID < 900
        if strcmpi(headers{i},'H') == 0
            tfv_data.(headers{i}) = create_interpolated_dataset...
                (data.(sites{site_ID}),headers{i},'Surface',datearray);
        else
            tfv_data.(headers{i}) = create_interpolated_dataset_hires...
                (data.(sites{site_ID}),headers{i},'Surface',datearray);
        end
    else
    ...
    if strcmpi(headers{i},'WQ_LND_TRACER') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 0;
    end
    
    if strcmpi(headers{i},'WQ_NIT_N2O') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 0.001;
    end

    if strcmpi(headers{i},'WQ_ZOO_GRAZER') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 0.000;
    end
    
    if strcmpi(headers{i},'WQ_GEO_PE') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 8;
    end
    
    if strcmpi(headers{i},'WQ_SBG_TRACER') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 0;
    end
    
    if strcmpi(headers{i},'WQ_SBG_DOM_LEACHATE') == 1
        tfv_data.(headers{i})(1:length(datearray),1) = 0;
    end    
...
function var = create_interpolated_dataset_hires(data,varname,depth,mtime)
% A Function to take a AED data structure, site and variable and create an
% interpolated dataset for use in creation of the input files
disp('Hi-Res data for H');
...
tt_date = data.(varname).Date;

u_date = unique(tt_date);

t_data(1:length(u_date)) = tt_data;%NaN;
t_date(1:length(u_date)) = tt_date;% NaN;
% for iii = 1:length(u_date)
%     [min_val,sss] = min(abs(tt_date - u_date(iii)));
%     
% %     switch depth
