function Clouds = calc_Cloud_Cover(startdate,enddate,TZ,Lat,Lon,SP)
...
sss = find(mDate_A >= startdate & mDate_A <= enddate);
mDate = mDate_A(sss);
SolRad = SP.SolRad(sss);
...
for i = 1:length(GHI_Interp)
    if SolRad(i) > GHI_Interp(i)
        SolRad(i) = GHI_Interp(i);
    end
end
