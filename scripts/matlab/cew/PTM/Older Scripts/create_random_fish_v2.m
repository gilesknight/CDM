...
total_cells = 3000; %Total Number of wet cells to contain fish (total number of BC files);
...
outdir = ['Output/',Domain,'_',num2str(biomass),'/'];
...
[~,~,~,~,~,X,Y,Z,ID,MAT,A] = tfv_get_node_from_2dm(['Grids/',Domain,'.2dm']);

sss = find(Z <= initial_height);

% Randomly choose wet cells upto the number of BC files wanted
if length(sss > total_cells)
    for i = 1:total_cells
        tCells_Num(i,1) = i;
        tCells_cellID(i,1) = datasample(ID(sss),1);
    end
else
    stop; % IF less than total cells will be wet
end
