    ...
    for i = 3:length(dirlist)
        ...
        if strcmpi(str{1},'Output') == 1
            if exist([main_dir,dirlist(i).name,'/run.nc'],'file')
                if ~exist([main_dir,dirlist(i).name,'/proc_cyano.mat'],'file')
                disp([main_dir,dirlist(i).name,'/run.nc']);
                
                data = tfv_readnetcdf([main_dir,dirlist(i).name,'/run.nc'],'names',{'D';'TEMP';'WQ_NIT_NIT';'WQ_NIT_AMM';'WQ_PHS_FRP';'cell_A';'cell_X';'cell_Y';'idx2'});
                clear functions
                ...
                save([main_dir,dirlist(i).name,'/proc_cyano.mat'],'data','-mat');
                end
