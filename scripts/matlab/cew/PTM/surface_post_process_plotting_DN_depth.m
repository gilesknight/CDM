...
for kk = 5%3:length(sitelist)
    ...
    fid = fopen([outputdirectory,'DN.csv'],'wt');
    ...
    base = load([base_dir,'proc_DN.mat']);
    ...
    for i = 3:length(dirlist)
        ...
        if strcmpi(str{1},'Output') == 1
            ...
            sim = load([main_dir,dirlist(i).name,'/proc_multi.mat']);
            ...
            for k = 1:length(shp)
                ...
                numpol = find(inpol == 1);
                for l = 1:size(sim.data.DN_SUF,2)
                    sss = find(Ddata.D(numpol,l) > 0.037 & Ddata.D(numpol,l) < 0.5 );
                    ttt = find(Ddata.D(numpol,l) >= 0.5);
                    if isempty(sss)
                        sss = find(Ddata.D(numpol,l) >= 0.5);
                    end
%                     DO(l) = sum(sim.data.DN_DIFF(numpol(sss),l)) / length(sss);
                    DOS(l) = sum(sim.data.DN_SUF(numpol(sss),l)) / length(sss);
                    DOB(l) = sum(sim.data.DN_SUF(numpol(ttt),l)) / length(ttt);
                end
%                 oxy_data(inc,k) = sum(DO)/length(DO) * -1;
                gbg = find(sim.data.tdate >= (min(sim.data.tdate) + 10) & ...
                    sim.data.tdate <= (min(sim.data.tdate) + 17));
                oxy_data_s(inc,k) = sum(DOS(gbg))/length(gbg) * -1;
                oxy_data_b(inc,k) = sum(DOB(gbg))/length(gbg) * -1;
