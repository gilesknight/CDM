    ...
    for i = 3:length(dirlist)
        ...
        if strcmpi(str{1},'Output') == 1
            ...
            for k = 1:length(shp)
                ...
                numpol = find(inpol == 1);
                for l = 1:size(sim.data.OXY_SUF,2)
                    sss = find(Ddata.D(numpol,l) > 0.1 & Ddata.D(numpol,l) < 0.5 );
                    ...
                    DOB(l) = sum(sim.data.OXY_SUF(numpol(ttt),l)) / length((ttt));
                ...
                oxy_data_s(inc,k) = sum(DOS(gbg))/length(gbg);
                oxy_data_b(inc,k) = sum(DOB(gbg))/length(gbg);
