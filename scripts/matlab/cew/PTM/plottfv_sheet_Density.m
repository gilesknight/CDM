...
for i = intBB:1:length(timesteps)%1:plot_interval:length(timesteps)
    ...
    ucells = unique(tdat.idx2);
    for bb = 1:length(ucells)
        ttt = find(tdat.idx2 == ucells(bb));
        
%         if sum(tdat.(varname)(ttt)) > 0
%             stop
%         end
        
        cdata(bb,1) = sum(tdat.(varname)(ttt)) / tdat.cell_A(bb) * 10000 * 1/1000;
    end
