...
mod(1).mat = 'D:\Studysites\Lowerlakes\CEWH_2016\noCEW_Barrage_T1\Output\Flux.mat';
mod(1).name = 'T1';
mod(1).color = 'b';

mod(2).mat = 'D:\Studysites\Lowerlakes\CEWH_2016\noCEW_Barrage_T2\Output\Flux.mat';
mod(2).name = 'T2';
mod(2).color = 'k';

% mod(3).mat = 'D:\Studysites\Lowerlakes\012_LL_CEWH_2015_noALL_SEDZONES_AED2_SS1_SAL\Output\Flux.mat';
% mod(3).name = 'Without All';
% mod(3).color = 'r';


% 
% mod(4).mat = 'E:\Studysites\RiverMurray\CEWH_2014\Simulations\LL_CEWH_2014_EWater_Cell_SedZones_v17\Output\Flux.mat';
% mod(4).name = 'Without CEW Without TLMW';
% mod(4).color = 'g';
...
outputdirectory = 'Y:\Studysites\Lowerlakes\CEWH_2015\Matlab\Plotting\Images\014_LL_CEWH_2015_OBS_SEDZONES_NoAED2_SS1_SAL\Flux Ave\';

timeperiod = 2*60*60;

[snum,str] = xlsread('matfiles/Flux Order WQ Ave TFV.xlsx','A2:H50');
...
for i = 1:length(sites)
    ...
    vars = fieldnames(data(2).flux.(sites{i}));
...
%     end
    ...
    for j = 1:length(vars)
        ...
        if strcmp(vars{j},'mDate') == 0
                ...
                for jj = 1:length(u_years)
                    for ii = 1:length(u_months)
                        ...
                        if ~isempty(ss)
                            ...
                            if inc == 1 % Monthly Average Monthly Total
                                
                                plotdata(1,jjj) = (sum(ydata(ss(tt)) .* timeperiod))/length(tt);
                                
                                
                            else
                                
                                plotdata(inc,jjj) = (sum(ydata(ss(tt)) .* timeperiod))/length(tt);
            ...
            ylabel(['Monthly Average ',labels{sss},' Load ',' (',units{sss},')'],'FontSize',7,'FontName','Arial');
