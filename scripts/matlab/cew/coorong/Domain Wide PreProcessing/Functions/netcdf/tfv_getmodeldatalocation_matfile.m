function data = tfv_getmodeldatalocation_matfile(filename,rawData,X,Y,varname)
...
geo_x = double(rawData.savedata.X);
geo_y = double(rawData.savedata.Y);
...
%     data.bottom  = rawData.(varname{1})(botIndex,:);
    ...
    data.surface = rawData.savedata.(varname{1}).Bot(pt_id,:);
    data.bottom  = rawData.savedata.(varname{1}).Bot(pt_id,:);
...
data.date = rawData.savedata.Time;%cont.netcdf.Interval(1:length(data.surface));
