...
%%%%%%%%%%%%%%%%%%''
hfig = figure('visible','on','position',[2.7497e+03 406.3333 0.6513e+03 1207.3333]);
set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
%set(gcf,'paperposition',[0.635 6.35 10.32 30.24])



 axes('position',[-0.25 -0.1 1.4 1.4]);
 ...
 xlim([3.4832e+05 3.7123e+05]);
 ylim([6.0224e+06 6.0330e+06]);
 camroll(-25)
 
 
 axes('position',[-0.2 -0.3 1.1 1.1]);
    patFig1 = patch('faces',faces,'vertices',vert,'FaceVertexCData',max_data(:,4));shading flat
    set(gca,'box','on');
    set(findobj(gca,'type','surface'),...
        'FaceLighting','phong',...
 ...
 xlim([3.6152e+05 3.8036e+05]);
 ylim([6.0004e+06 6.0122e+06]);
 camroll(-25)
 
 
  axes('position',[ 0.07 0.26 1.1 1.1]);
    patFig1 = patch('faces',faces,'vertices',vert,'FaceVertexCData',max_data(:,4));shading flat
    set(gca,'box','on');
    set(findobj(gca,'type','surface'),...
        'FaceLighting','phong',...
 ...
 xlim([3.2511e+05 3.4059e+05]);
 ylim([6.0406e+06 6.0503e+06]);
 camroll(-25)
 
 
axes('position',[ -0.3 0.0  1.8 1.0]);
%axes('position',[ 0.0 0.0  0.5 1]);
    ...
    axis off;%set(gca,'xticklabel',[],'yticklabel',[]);
    cb = colorbar('location','South','orientation','horizontal');
    set(cb,'position',[0.05 0.95 0.25 0.01]);
...
yTop = (30-ySize)/2;
%set(gcf,'paperposition',[0 0 xSize ySize])
set(gcf,'paperposition',[0.635 6.35 10.32 20.24])
