...
function [HSI,fS,fT,fO,fA]=cal_HSI_mulloway(sal_bot,monthTEMP, Depth)%,do_bot,amm_bot,)
%load swan_WQ.mat;
    ...
    for jj=1:size(sal_bot,2) %time step
        ...
        if Depth(ii,jj)<0.1
            ...
            elseif monthTEMP(ii,jj)<t2
                s10=((51-60)/(23-14))*(monthTEMP(ii,jj)-14)+60;
                s50=((59-64)/(23-14))*(monthTEMP(ii,jj)-14)+64;
            elseif monthTEMP(ii,jj)>=t2
                s10=51;
                s50=59;
            end
