...
function [HSI,fS,fT,fO,fA]=cal_HSI_congolli(sal_bot,monthTEMP, Depth)%,do_bot,amm_bot,)
%load swan_WQ.mat;
    ...
    for jj=1:size(sal_bot,2) %time step
        ...
        if Depth(ii,jj)<0.1
            ...
            elseif monthTEMP(ii,jj)<t2
                s10=((87-90)/(23-14))*(monthTEMP(ii,jj)-14)+90;
                s50=((94-100)/(23-14))*(monthTEMP(ii,jj)-14)+100;
            elseif monthTEMP(ii,jj)>=t2
                s10=87;
                s50=94;
            end
